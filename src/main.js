...
function renderLogin() {
  ...
  shell(`<main class="login-page login-light"><section class="login-copy"><div class="login-welcome"><p class="eyebrow">YASNOLAND TRACKER</p><h1>${ru ? 'Рады видеть<br><em>тебя снова!</em>' : 'Good to see<br><em>you again!</em>'}</h1><p>${ru ? 'Твои награды и учебный прогресс — в одном удобном кабинете.' : 'Your rewards and learning progress, together in one simple space.'}</p></div></section><section class="login-card"><div class="login-language" role="group" aria-label="Language"><button class="${ru ? 'active' : ''}" data-login-language="ru">Русский</button><button class="${!ru ? 'active' : ''}" data-login-language="en">English</button></div><p class="eyebrow">${ru ? 'ВХОД ДЛЯ УЧЕНИКА' : 'STUDENT ACCESS'}</p><h2>${ru ? 'Войти в кабинет' : 'Sign in'}</h2><p class="muted">${ru ? 'Введите логин и пароль ученика.' : 'Enter the student login and password.'}</p><form id="login-form"><label>${ru ? 'Логин ученика' : 'Student login'}<input name="id" autocomplete="username" value="${esc(rememberedStudentId())}" placeholder="${ru ? 'Например, Y001' : 'For example, Y001'}" required></label><label>${ru ? 'Пароль / PIN-код' : 'Password / PIN'}<input name="pin" type="password" inputmode="numeric" autocomplete="current-password" placeholder="${ru ? 'Введите пароль' : 'Enter password'}" required></label><label class="remember-login"><input name="remember" type="checkbox" checked><span>${ru ? 'Запомнить вход на этом устройстве' : 'Remember me on this device'}</span></label><p id="login-error" class="form-error" role="alert"></p><button class="primary" type="submit">${ru ? 'Войти' : 'Sign in'} <span>→</span></button></form><div class="login-divider"><span>${ru ? 'или' : 'or'}</span></div><button class="leaderboard-button" data-action="leaderboard">${ru ? 'Посмотреть рейтинг учеников' : 'View student leaderboard'}</button><p class="security-note">${ru ? 'Не используйте общий компьютер для сохранения входа.' : 'Do not remember your login on a shared computer.'}</p></section></main><footer>Ясноленд · Где английский становится понятным</footer>`);
  document.querySelectorAll('[data-login-language]').forEach(button => button.addEventListener('click', () => { uiLanguage = button.dataset.loginLanguage; try { localStorage.setItem(LANGUAGE_KEY, uiLanguage); } catch {} renderLogin(); }));
  document.querySelector('#login-form').addEventListener('submit', event => {
    ...
    currentStudent = student; values.get('remember') ? rememberStudent(student.id) : forgetStudent(); renderDashboard();
  });
}
